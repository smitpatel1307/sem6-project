...
<body>
				...
				<form action="search.php" method="post">
                    <div class="atbd_generic_header">
                        <div class="atbd_generic_header_title">
							...
							$sql = $link->rawQueryOne('select count(v.vendor_id) as cnt from vendor v,contact c,about a,category ct where v.category_id=ct.category_id and v.vendor_id=c.vendor_id and v.vendor_id=a.vendor_id and v.approve=? and v.category_id=?',Array("2",$_GET['cid']));
							if($link->count > 0)
							{
								$count = $sql['cnt'];
							}
							$rows_page=1; //record per page...
   
						   $last=ceil($count / $rows_page);
						...
						<div class="form-group">
							<input type="text" placeholder="What are you looking for?" class="form-control" style="width: 250px;" name="look" id="look">
						</div>
	...
	<?php
				include 'include/connect.php';
				$cid=$_GET['cid'];
				$sql = $link->rawQuery('select cg.category_id,v.vendor_id,v.shop_name,c.address,c.phone_no1,c.phone_no2,a.baner_image from vendor v,contact c,about a,category cg where v.category_id=cg.category_id and v.vendor_id=c.vendor_id and v.vendor_id=a.vendor_id and v.approve=? and v.category_id=? limit ?,?',Array("2",$cid,$si,$rows_page));
				if($link->count > 0)
				{
					foreach($sql as $s)
					{
										...
										<div class="atbd_listing_bottom_content">
											<div class="atbd_content_left">
												<div class="atbd_listing_category">
													...
													?>
													<a href="../Admin-Panel/theme/upload/<?php echo $sql['theme_url'];?>?vendor_id=<?php echo $s['vendor_id']; ?>" target="_private"><span class="la la-eye"></span>Website</a>
												</div>
											</div>
										</div>
									<?php
        ...
        <div class="nav-links">
			...
			for($i=1;$i<=$last;$i++)
			{
				...
				<a class="page-numbers <?php 
				                       ...
				                       ?>" href='all-listings-list.php?page_num=<?php echo $i; ?>'><?php echo $i; ?></a>
				<?php
			}
			?>
        </div>
