...
if(isset($_POST['submit']))
{
	...
	$s=$link->rawQueryOne('select * from slider where slider_id=?',Array($slider_id));
    if($link->count > 0)
	{
		$slider_image=$s['slider_image'];
	}
		$icon = $_FILES["simage"]["name"];
		$ext = pathinfo($icon,PATHINFO_EXTENSION);
		$name = "slider".$slider_id.".".$ext;
		if(move_uploaded_file($_FILES["simage"]["tmp_name"],'image/'.$name))
		{
			$link->where('slider_id',$slider_id);
			$ui = $link->update('slider',Array('slider_image'=>$slider_image));
			if($ui)
			{
...
<head>
  ...
  <title>QuickSearch|Vendor|Update Slider</title>
  <!-- plugins:css -->
				  ...
				  <div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group col-xs-12">
                        <input type="text" class="form-control file-upload-info" value="<?php echo $slider_image;?>" disabled>
                        <span class="input-group-append">
                          <button class="file-upload-browse btn btn-primary" type="button">Upload</button>
                        </span>
                      </div>
					   <div class="input-group col-xs-12">
                        <img src="<?php echo $slider_image; ?>" height="50px" width="50px">
                      </div>
